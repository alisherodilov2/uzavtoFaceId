...
FileInfo.prototype.draw = function(g)
{
	if (this.isInRange(this.m_ixMin, this.m_ixMax))
	{
		...
		{
			g.fillRect(this.m_iX, this.m_iY - 3, this.m_iWidth, this.m_iHeight + 2);
		}
		else if ((this.m_iX < this.m_ixMax) && ((this.m_iX+this.m_iWidth) > this.m_ixMax))
		{
			g.fillRect(this.m_iX, this.m_iY - 3, this.m_ixMax - this.m_iX, this.m_iHeight + 2);
		}
		else
		{
			g.fillRect(this.m_ixMin, this.m_iY - 3, (this.m_iX+this.m_iWidth) - this.m_ixMin, this.m_iHeight + 2);
		}
 ...
 * @created 2011-04-06
 * @version v1.0
 * @function 工具类，时间条 iType 时间条模式 0 一项, 1 两项
 *************************************************/
function TimeBar(canvas, iType/*, iWidth, iHeight*/)
{
	this.m_showMode = 0; //时间条模式
	this.m_iSelectItem = 0;//当前选择时间条
	if(arguments.length >= 4)
	{
		this.m_showMode = iType;
	...
	this.partLineColor = 'rgb(0,0,0)';          //分割线颜色
	this.channelNameColor = 'rgb(150, 150, 150)';   //通道名称颜色
	this.timeScaleColor = '#cccccc';     //时间条刻度颜色
	this.middleLineColor = 'red';      //中轴线颜色 kevin
	...
	this.scheFileColor = '#2196F3';      //录像计划颜色
	this.alarmFileColor = 'rgb(248, 71, 126)';     //警告录像颜色
	this.manualFileColor = 'rgb(247, 199, 5)';      //手动录像颜色
	this.secondItemFileColor = 'rgb(255, 0, 0)';    //第二条录像颜色(智能后检索)
	...
	this.FileInfoSetEx = new Array(this.m_iMaxWndNum); //智能文件信息集合
	//初始化文件信息集合
	for(i = 0; i < this.m_iMaxWndNum; i++)
	{
		this.FileInfoSet[i] = new Array();
		this.FileInfoSetEx[i] = new Array();
	}
	...
	this.m_iFileRectHeight = 12; //文件块的高度
	...
	this.Stop = function()
	{
		...
		//this.m_canvas.style.cursor="url(./thirdLib/timebar/H_point1.cur),pointer";
		this.m_bMOuseDown = false;
		if(this.m_iMove === 0) {
	...
	{
		// document.body.style.cursor="url(./thirdLib/timebar/H_point.cur),pointer";
		// this.m_canvas.style.cursor="url(./thirdLib/timebar/H_point.cur),pointer";
	...
	this.GetSelectItemID = function()
	{
		if(this.m_ieventY && this.m_showMode == 1)
		{
			var iTopPoston = this.m_iCanvasTop + this.m_iTimeRectHeight + parseInt(this.m_iBlankHeight/2);
			if(this.m_ieventY >= iTopPoston && this.m_ieventY <= (iTopPoston + parseInt(this.m_iFileRectHeight/2)))
			{
				this.m_iSelectItem = 0;
			}
			else if(this.m_ieventY >= (iTopPoston + parseInt(this.m_iFileRectHeight/2)) && this.m_ieventY <= (iTopPoston + this.m_iFileRectHeight))
			{
...
TimeBar.prototype.repaint = function()
{
    ...
    this.m_ctx.lineWidth = 11;
	this.m_ctx.beginPath();
	this.m_ctx.moveTo(0, 44);
	this.m_ctx.lineTo(this.m_iWidth, 44);
	this.m_ctx.stroke();
	...
	//画文件信息区域
	//console.log(this.FileInfoSet);
	for(i = 0; i < this.FileInfoSet[this.m_iSelWnd].length; i++)
	{
		this.FileInfoSet[this.m_iSelWnd][i].draw(this.m_ctx);
    }
    //智能检索增加颜色相关 到时间条  kevin
	if(this.m_showMode == 1) {
		//画文件信息区域
		for(i = 0; i < this.FileInfoSetEx[this.m_iSelWnd].length; i++)
		{
			this.FileInfoSetEx[this.m_iSelWnd][i].draw(this.m_ctx);
		}
	}
    //画时间刻度
	for (i = 0; i < this.ScaleInfoNum; i++)
	{
		...
		{
            this.m_ctx.beginPath();
            this.m_ctx.moveTo(this.ScaleInfo[i].m_ix,48); //this.m_iTimeRectHeight kevin
            this.m_ctx.lineTo(this.ScaleInfo[i].m_ix, 39); //this.m_iHeight kevin
			this.m_ctx.stroke();
            this.m_ctx.fillText(this.ScaleInfo[i].m_szTime, this.ScaleInfo[i].m_ix - 15, 30);
        }
        var iLength = (this.ScaleInfo[2].m_ix - this.ScaleInfo[1].m_ix)/5;
        //绘制5段分割线 kevin
        for (var j = 1; j < 5; j++) {
            this.m_ctx.beginPath();
            this.m_ctx.moveTo(this.ScaleInfo[i].m_ix+iLength*j,48); 
            this.m_ctx.lineTo(this.ScaleInfo[i].m_ix+iLength*j, 44);
            this.m_ctx.moveTo(this.ScaleInfo[i].m_ix-iLength*j,48); 
            this.m_ctx.lineTo(this.ScaleInfo[i].m_ix-iLength*j, 44);
            this.m_ctx.stroke();
        }
    ...
    //画中轴线 kevin  修改中轴线色值 以及高度
	this.m_ctx.strokeStyle = this.middleLineColor;
	this.m_ctx.lineWidth = 2;
	this.m_ctx.beginPath();
	this.m_ctx.moveTo(this.m_iMiddleLinePos, 30);
	this.m_ctx.lineTo(this.m_iMiddleLinePos,50);
    this.m_ctx.stroke();
...
TimeBar.prototype.updateFileListPos= function()
{
	...
	if(this.m_showMode == 1)
	{
		var iFileLength = this.FileInfoSetEx[this.m_iSelWnd].length;
		if(iFileLength == 0)
		{
			return;
		}
		var tStartTime = this.FileInfoSetEx[this.m_iSelWnd][0].m_tStartTime;
		var seconds = parseInt((tStartTime.m_iMilliseconds - this.m_tCurrentMidTime.m_iMilliseconds)/1000);
		var iFile0Pos = this.m_iMiddleLinePos + parseInt(parseFloat(seconds / (3600*this.m_fCellTime)) * this.m_iCellWidth);
		var iMoved = iFile0Pos - this.FileInfoSetEx[this.m_iSelWnd][0].m_iX;
		//没有移动直接返回
		if(iMoved == 0)
		{
			return;
		...
		for (i = 0; i < iFileLength; i++)
		{
			var iX = this.FileInfoSetEx[this.m_iSelWnd][i].m_iX + iMoved;
			var iY = this.FileInfoSetEx[this.m_iSelWnd][i].m_iY;
			var iWidth = this.FileInfoSetEx[this.m_iSelWnd][i].m_iWidth;
			var iHeight = this.FileInfoSetEx[this.m_iSelWnd][i].m_iHeight;
			this.FileInfoSetEx[this.m_iSelWnd][i].setPos(iX, iY, iWidth, iHeight);
		}
	}
}
...
TimeBar.prototype.resize = function(iWidth, iHeight)
{
	...
	if(this.m_showMode == 1) {
		...
		for (i = 0; i < this.FileInfoSet[this.m_iSelWnd].length; i++)
		{
			...
			FileInfoSetSel.setPos(iFilePosLeft, this.m_iTimeRectHeight + parseInt(this.m_iBlankHeight/2), iFilePosRight - iFilePosLeft, 12);
		}
		//初始化文件列表信息
		for (i = 0; i < this.FileInfoSetEx[this.m_iSelWnd].length; i++)
		{
			var FileInfoSetSel = this.FileInfoSetEx[this.m_iSelWnd][i];
			var iXLeftSeconds = parseInt((FileInfoSetSel.m_tStartTime.m_iMilliseconds - this.m_tCurrentMidTime.m_iMilliseconds)/1000);
			var iFilePosLeft = this.m_iMiddleLinePos + parseInt(parseFloat(iXLeftSeconds / (3600*this.m_fCellTime)) * this.m_iCellWidth);
...
Function:		addFile
Description:	添加文件
Input:			StartTime 开始时间, StopTime 结束时间, iType 类型, iElementId 项编号, iWndNum 默认当前窗口 添加到某个窗口
Output:			无
return:			无
*************************************************/
TimeBar.prototype.addFile = function(StartTime, StopTime, iType, iElementId, iWndNum)
{
	//console.log("TimeBar.prototype.addFile: StartTime = %s StopTime = %s", StartTime, StopTime);
	...
	switch (iType)
	{
		...
		case 4:
			...
			break;
		case 5:
			fileColor = this.secondItemFileColor;
			break;
		default:
	...
	{
		if(iElementId == 1) {
			var fileInfo = new FileInfo(iFilePosLeft, this.m_iTimeRectHeight + parseInt(this.m_iBlankHeight/2), iFilePosRight - iFilePosLeft, 12, iType, fileColor, tStartTime, tStopTime);
			fileInfo.setPosRange(this.m_iFileListStartPos, this.m_iFileListStartPos + parseInt(this.m_iCellWidth * this.ScaleInfoNum));
			if("undefined" !== typeof iWndNum)
			{
				this.FileInfoSetEx[iWndNum].push(fileInfo);
			}
			else
			{
				this.FileInfoSetEx[this.m_iSelWnd].push(fileInfo);
			}
		}
		else
		{
			...
			if("undefined" !== typeof iWndNum)
			{
				this.FileInfoSet[iWndNum].push(fileInfo);
			}
			else
			{
...
return:			无
*************************************************/
TimeBar.prototype.clearWndFileList = function(iWndNum, iElement)/*iWndNum*/
{
	//console.log("TimeBar.prototype.clearWndFileList 窗口号 = %s 默认当前选中窗口 = %s", iWndNum, iElement);
	var iWndParam;
	...
	if(arguments.length == 2) {
		if(0 == arguments[1]) {
			...
			this.FileInfoSetEx[iWndParam].length = 0;
		}
	} else {
		this.FileInfoSet[iWndParam].length = 0;
		this.FileInfoSetEx[iWndParam].length = 0;
...
/*************************************************
 ...
 Description:	获取选中项编号
 Input:			null
 Output:		m_iSelectItem 项编号
 return:		无
 *************************************************/
TimeBar.prototype.getSelectItemID = function()
{
